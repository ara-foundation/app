...
type ReversedHoleBackgroundProps = React.ComponentProps<'div'> & {
  strokeColor?: string;
...
function ReversedHoleBackground({
  strokeColor = '#737373',
  numberOfLines = 50,
  numberOfDiscs = 50,
  ...
  const initParticle = React.useCallback(
    (start: boolean = false) => {
      ...
      const y = start
        ...
        : 0; // Start from top instead of bottom
  ...
  const drawDiscs = React.useCallback(
    (ctx: CanvasRenderingContext2D) => {
      ctx.strokeStyle = strokeColor;
      ctx.lineWidth = 2;
      const outerDisc = stateRef.current.endDisc; // Use endDisc as the outer disc
      ctx.beginPath();
      ctx.ellipse(
        outerDisc.x,
        outerDisc.y,
  ...
  const moveDiscs = React.useCallback(() => {
    stateRef.current.discs.forEach((disc: any) => {
      // Reverse direction: go from 1 to 0 instead of 0 to 1
      disc.p = (disc.p - 0.001 + 1) % 1;
      tweenDisc(disc);
    });
  }, [tweenDisc]);
  ...
  const moveParticles = React.useCallback(() => {
    stateRef.current.particles.forEach((particle: any, idx: number) => {
      ...
      // Reverse particle movement: particles move upward (coming out)
      particle.y += particle.vy;
      if (particle.y > stateRef.current.particleArea.h) {
  ...
  return (
    <div
      data-slot="reversed-hole-background"
      className={cn(
        ...
        'after:content-[""] after:absolute after:z-[5] after:top-1/2 after:left-1/2 after:block after:size-full after:[background:radial-gradient(ellipse_at_50%_25%,#a900ff_20%,transparent_75%)] after:[transform:translate3d(-50%,-50%,0)] after:mix-blend-overlay',
        className,
      )}
      {...props}
    >
      ...
      {/* Squished grid to create the "coming out" effect */}
      <div
        aria-hidden
        className={cn(
        ...
        style={{
          transform: 'perspective(900px) rotateX(-72deg) scale(1.05)', // Flipped rotation
          transformOrigin: '50% 60%', // Adjusted origin
          maskImage: 'radial-gradient(ellipse at 50% 45%, black 35%, transparent 75%)', // Flipped mask position
          WebkitMaskImage: 'radial-gradient(ellipse at 50% 45%, black 35%, transparent 75%)',
          filter: 'blur(0.6px)',
        }}
      />
...
export { ReversedHoleBackground, type ReversedHoleBackgroundProps };
